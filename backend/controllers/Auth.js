...
exports.resendOtp=async(req,res)=>{
    try {
        ...
        // Ensure OTP_EXPIRATION_TIME is a valid number, fallback to 300000 (5 minutes) if not set
        const otpExpirationTime = parseInt(process.env.OTP_EXPIRATION_TIME)
        const expiresAt = Date.now() + (isNaN(otpExpirationTime) ? 300000 : otpExpirationTime)

        const newOtp=new Otp({user:req.body.user,otp:hashedOtp,expiresAt:expiresAt})
        await newOtp.save()
...
exports.forgotPassword=async(req,res)=>{
    ...
    try {
        ...
        const expirationTime = parseInt(process.env.OTP_EXPIRATION_TIME);
if (isNaN(expirationTime)) {
  throw new Error('Invalid OTP expiration time configuration');
}
newToken=new PasswordResetToken({user:isExistingUser._id,token:hashedToken,expiresAt:Date.now() + expirationTime})
        await newToken.save()
