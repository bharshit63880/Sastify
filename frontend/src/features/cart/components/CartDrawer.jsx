      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-5">
        {isPending && !adapted.length ? (
          ...
          <div role="alert" className="flex min-h-64 flex-col items-center justify-center gap-3 text-center">
            <p className="font-semibold text-primary">We couldn’t load your cart</p>
          ...
          <div className="flex min-h-64 flex-col items-center justify-center gap-4 text-center">
            ...
            <div><p className="text-xl font-semibold text-primary">Your cart is empty</p><p className="mt-1 text-small">Explore the catalog to add your first item.</p></div>
                ...
                <div className="min-w-0">
                  <div className="flex items-start gap-2">
                    <button type="button" onClick={() => go(`/products/${item.slug}`)} className="min-w-0 flex-1 text-left">
                      <span className="line-clamp-2 text-sm font-semibold text-primary">{item.name}</span>
                      {item.brand ? <span className="mt-0.5 block text-xs text-secondary">{item.brand}</span> : null}
                    </button>
                  ...
                  {(item.size || item.color) ? <p className="mt-2 text-xs text-secondary">{[item.color && `Color: ${item.color}`, item.size && `Size: ${item.size}`].filter(Boolean).join(" · ")}</p> : null}
                  <div className="mt-3 flex items-center justify-between gap-3">
                    <div className="neumorphic-control inline-flex items-center rounded-pill p-1">
                      ...
                      <span className="min-w-8 text-center text-sm font-semibold text-primary" aria-live="polite">{item.quantity}</span>
                    ...
                    <strong className="text-sm text-primary">{formatPrice(item.price * item.quantity)}</strong>
                  </div>
                </div>
              </article>
            ))}
      ...
      {adapted.length ? (
        <div className="border-t border-default bg-surface-glass p-5">
          <div className="mb-4 space-y-2 text-sm">
            <div className="flex justify-between text-secondary"><span>Subtotal</span><strong className="text-primary">{formatPrice(subtotal)}</strong></div>
            <div className="flex justify-between text-secondary"><span>Shipping</span><span>{shipping ? formatPrice(shipping) : "Free"}</span></div>
          </div>
          <div className="grid gap-2">
            ...
            <button type="button" onClick={closeOverlay} className="py-2 text-sm font-semibold text-secondary hover:text-primary">Continue shopping</button>
          </div>
        </div>
      ) : null}
    </Drawer>
