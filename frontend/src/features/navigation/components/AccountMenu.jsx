...
export const AccountMenu = ({ open, user, onClose, onLogout }) => {
  ...
  return (
    <div className="absolute right-0 top-[calc(100%+10px)] w-64 rounded-2xl border border-glass bg-glass p-3 shadow-lg backdrop-blur-2xl" role="menu">
      {user ? <div className="border-b border-default px-3 pb-3"><p className="truncate font-semibold text-text-primary">{user.name || "Your account"}</p><p className="truncate text-xs text-text-secondary">{user.email}</p></div> : null}
      <div className="py-2">
        {links.map((item) => <Link role="menuitem" key={item.to} to={item.to} onClick={onClose} className="block rounded-xl px-3 py-2.5 text-sm font-medium text-text-secondary hover:bg-surface-muted hover:text-text-primary">{item.label}</Link>)}
      </div>
