    ...
    <Dialog open={Boolean(quickViewProduct)} onClose={closeQuickView} title={`Quick view: ${label}`} fullScreenMobile className="max-w-5xl">
      <div className="flex h-full max-h-[90vh] flex-col bg-surface max-sm:max-h-none">
        <div className="flex items-center justify-between border-b border-default px-5 py-4">
          <div><p className="text-label text-brand-primary">Quick view</p><p className="font-semibold text-primary">{label}</p></div>
        ...
        {status === "pending" && !product ? (
          ...
          <div className="flex min-h-80 flex-1 flex-col items-center justify-center gap-4 p-6 text-center" role="alert">
            <p className="text-xl font-semibold text-primary">Quick view is unavailable</p>
          ...
          <div className="grid min-h-0 flex-1 overflow-y-auto md:grid-cols-[1.02fr_.98fr]">
            ...
            <div className="flex flex-col gap-5 p-5 sm:p-7">
              <div>
                ...
                <h2 className="mt-2 text-3xl font-bold tracking-[-.04em] text-primary">{label}</h2>
                <div className="mt-3 flex items-center gap-2 text-sm text-text-secondary"><FiStar className="fill-current text-warning" /><span className="font-semibold text-primary">{Number(product.ratingAverage || product.rating || 0).toFixed(1)}</span><span>({Number(product.ratingCount || product.reviewCount || 0)} reviews)</span></div>
              </div>
              <div className="flex flex-wrap items-end gap-3"><strong className="text-3xl text-primary">{formatPrice(product.price)}</strong>{Number(product.originalPrice) > Number(product.price) ? <span className="text-text-secondary line-through">{formatPrice(product.originalPrice)}</span> : null}</div>
