...
export const WishlistPage = () => {
  ...
  const availableItems = getAvailableWishlistItems(items);
  ...
  return (
    <PageWrapper className="py-8">
      ...
      {status === "pending" ? <SkeletonRegion label="Loading wishlist"><div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-4">{[1, 2, 3, 4].map((key) => <ProductCardSkeleton key={key} />)}</div></SkeletonRegion> : availableItems.length ? (
        <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-4">{availableItems.map((item) => {
